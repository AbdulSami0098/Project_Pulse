...
export const handlePushEvent = async (payload: Record<string, unknown>, projectId?: number) => {
  const repo = (payload.repository as Record<string, unknown>)?.name as string ?? 'unknown';
  const pid = projectId ?? await getOrCreateProject(repo);
  ...
  return createEvent(pid, 'push', 'github', {
    ref: payload.ref,
...
export const handlePullRequestEvent = async (payload: Record<string, unknown>, projectId?: number) => {
  const repo = (payload.repository as Record<string, unknown>)?.name as string ?? 'unknown';
  const pid = projectId ?? await getOrCreateProject(repo);
  const pr = payload.pull_request as Record<string, unknown> | undefined;
  const user = pr?.user as Record<string, unknown> | undefined;

  return createEvent(pid, 'pull_request', 'github', {
